...
/**
 * Created by daireker on 2017/11/8.
 */
...
public class ClearEditText extends android.support.v7.widget.AppCompatEditText implements View.OnFocusChangeListener, TextWatcher{
    ...
    private void init() {
        //获取EditText的DrawableRight，加入没有设置就使用默认的图片
        mClearDrawable = getCompoundDrawables()[2];
        if(mClearDrawable == null) {
            //缺少资源文件图片，设置的另外一张
            mClearDrawable = getResources().getDrawable(R.drawable.base_action_bar_back_normal);
        }
    ...
    /*因为不能直接给EditText设置点击事件，所以需用记住按下的位置来模拟点击事件
    当我们按下的位置在 EditText的宽度 -图标到控件右边的间距 - 图标的宽度
    ...
    //当ClearEditText焦点发生变化时，判断里面字符串长度设置清除图标的显示与隐藏
    @Override
